function [TC,vals,vecs] = particlemotion(TC, varargin)
...
% $Date$
% $Revision$
% Changed by Alex Farrell on 4/2018
%    Added ability to return all eigenvalues and eigenvectors
...
for n = 1:nMax;
    ...
    [TC(n).rectilinearity,TC(n).planarity,TC(n).energy,TC(n).azimuth,TC(n).inclination, vecs, vals] = do_one(TC(n),dt,width); 
end
fprintf('\n');
...
function [rec, plan, energy, az, incl, vecs, vals] = do_one(TC,dt,width)
