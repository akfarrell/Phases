...
TCS = threecomp(W_all_short,az);
TCR = TC.rotate();
TCSR = TCS.rotate();
TCW = TCR.waveform();


TCUS = threecomp(W_all_unfilt_short,az);
TCUSR = TCUS.rotate();
...
[azim, incd, ellip, rpol, ppol, tvecC_zs, F, Cxypzz, Cxypnn, Cxypee] = ...
...
clear phaseStruct
load('phaseStruct.mat')
phaseStruct.(sprintf('eq_%d',eq)).(stasz).incWest(indie) = tcrp_inc_data(val_try);
phaseStruct.(sprintf('eq_%d',eq)).(stasz).azWest(indie) = tcrp_az_data(val_try);
phaseStruct.(sprintf('eq_%d',eq)).(stasz).peak{indie} = which_pk;
phaseStruct.(sprintf('eq_%d',eq)).(stasz).planWest(indie) = plan_vals(val_try);
phaseStruct.(sprintf('eq_%d',eq)).(stasz).recWest(indie) = rec_vals(val_try);
phaseStruct.(sprintf('eq_%d',eq)).(stasz).ind(indie) = val_try;
try
    phaseStruct.(sprintf('eq_%d',eq)).(stasz).haney_az(indie) = azim(id_diff,val_try/100);
    phaseStruct.(sprintf('eq_%d',eq)).(stasz).haney_inc(indie) = incd(id_diff,val_try/100);
    phaseStruct.(sprintf('eq_%d',eq)).(stasz).haney_ellip(indie) = ellip(id_diff,val_try/100);
    phaseStruct.(sprintf('eq_%d',eq)).(stasz).haney_rect(indie) = rpol(id_diff,val_try/100);
    phaseStruct.(sprintf('eq_%d',eq)).(stasz).haney_plan(indie) = ppol(id_diff,val_try/100);
catch
    phaseStruct.(sprintf('eq_%d',eq)).(stasz).haney_az(indie) = interp1(tvecC_z,azim(id_diff,:),val_try/100);
    phaseStruct.(sprintf('eq_%d',eq)).(stasz).haney_inc(indie) = interp1(tvecC_z,incd(id_diff,:),val_try/100);
    phaseStruct.(sprintf('eq_%d',eq)).(stasz).haney_ellip(indie) = interp1(tvecC_z,ellip(id_diff,:),val_try/100);
    phaseStruct.(sprintf('eq_%d',eq)).(stasz).haney_rect(indie) = interp1(tvecC_z,rpol(id_diff,:),val_try/100);
    phaseStruct.(sprintf('eq_%d',eq)).(stasz).haney_plan(indie) = interp1(tvecC_z,ppol(id_diff,:),val_try/100);
end
