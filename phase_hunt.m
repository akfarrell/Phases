% function high_corrs = phase_hunt(allorids,oridStruct,P,fil,cutoff_val)
...
for count=203%1:numel(allorids)
    ...
    for count2 = 1:3:numel(w_clean) %43-45 is PLMN 43:3:43
        ...
        try
            ...
            ind_S = numel(Haystack_data.(ch{count3}));
        end
        ...
        try
            ...
            S_ind = numel(Haystack_data.(ch{count3}))-numel(needle.(ch{count3}));
        end
        for count3 = 1:numel(ch)
            Haystack_data.(ch{count3}) = get(w_clean(count2+count3-1),'data');
            needle.(ch{count3}) = P.(sprintf('data%s',ch{count3}));
            %fix indexing issues
